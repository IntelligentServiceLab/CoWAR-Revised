...
class SANFM(nn.Module):
    def __init__(self, embed_dim, att_dim, droprate, i_num, c_num):
        super(SANFM, self).__init__()

        self.i_num = i_num
        self.c_num = c_num
        self.embed_dim = embed_dim  # 暂定64
        self.att_dim = att_dim  # 暂定，用于att的输出维度，可修改
        self.bi_inter_dim = embed_dim  # 暂定，可修改
        self.droprate = droprate
        self.criterion = nn.BCELoss(weight=None, reduction='mean')
        self.sigmoid = nn.Sigmoid()

        self.dense_embed = nn.Linear((self.i_num + self.c_num),
                                     self.embed_dim)  # 将batch*1500+的转化为batch*64，一行为64维向量，达到dense的目的
    ...
    def forward(self, batch_data):  # 输出batch*103
        ...
        # MLP part
        mlp_hidden_1 = F.relu(
            self.bn(self.hidden_1(value_weight_sum)))  # 暂时不用Batch Normalization，更新，感觉必须得用，不然loss维持在0.69，娘希匹
