...
def recommend(positive_labeled_dataset:list, api_representation_dict:dict, valid_api_dict:dict, model_obj, model_poolingModel):
    ...
    # 将所有拼接好的特征向量放到attention block模块中进行加权求和处理
...
def calculate_average_mrr_recall(n, model_obj, model_poolingModel):
    ...
    with open('../data/api_representation_dict_sbert.pkl', 'rb') as f_load:
    ...
    # 计算平均值
    average_mrr = round(total_mrr / n, 4)
    average_recall = [round(r / n, 4) for r in total_recall]
...
if __name__ == '__main__':
    ...
    optimizer = torch.optim.Adam(model_cowar.parameters(), lr=lr)
    ...
    # torch.save(model_poolingModel.state_dict(), '../model/pooling_model_attention_sbert.pth')
    model_poolingModel.eval()  # 设置为评估模式

    model_sanfm = model_cowar.sanfm
    # torch.save(model_sanfm.state_dict(), '../model/sanfm_model_attention_sbert.pth')
    model_sanfm.eval()  # 设置为评估模式

    for _ in range(5):
        average_mrr, recall_list = calculate_average_mrr_recall(2310, model_sanfm, model_poolingModel)
